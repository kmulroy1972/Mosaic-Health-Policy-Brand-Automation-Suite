import { HttpRequest, HttpResponseInit, InvocationContext } from '@azure/functions';

import { authenticateRequest } from '../auth/middleware';
import { extractTraceContext, injectTraceContext } from '../telemetry/tracing';
import { createLogger } from '../utils/logger';

import { translateText, type TranslationRequest } from './service';


/**
 * HTTP trigger for translation service.
 * POST /api/translate
 */
export async function translateHttpTrigger(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  const traceContext = extractTraceContext(request);
  const logger = createLogger(context, request);

  if (request.method !== 'POST') {
    return {
      status: 405,
      jsonBody: { error: 'Method not allowed. Use POST.' },
      headers: injectTraceContext(traceContext)
    };
  }

  const authResult = await authenticateRequest(request, context, {
    requireAuth: true
  });

  if (!authResult.authenticated || !authResult.context) {
    return {
      ...(authResult.error || {
        status: 401,
        jsonBody: { error: 'Unauthorized' }
      }),
      headers: injectTraceContext(traceContext)
    };
  }

  try {
    const body = (await request.json()) as TranslationRequest;

    if (!body.text || !body.targetLanguage) {
      return {
        status: 400,
        jsonBody: { error: 'Missing required fields: text, targetLanguage' },
        headers: injectTraceContext(traceContext)
      };
    }

    logger.info('Translation requested', {
      sourceLanguage: body.sourceLanguage || 'en',
      targetLanguage: body.targetLanguage,
      textLength: body.text.length,
      correlationId: traceContext.correlationId
    });

    const result = await translateText(body, context);

    return {
      status: 200,
      jsonBody: result,
      headers: {
        'Content-Type': 'application/json',
        ...injectTraceContext(traceContext)
      }
    };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error ?? 'unknown');
    logger.error('Translation failed', error instanceof Error ? error : new Error(errorMessage));

    return {
      status: 500,
      jsonBody: {
        error: 'Translation failed.',
        details: errorMessage
      },
      headers: injectTraceContext(traceContext)
    };
  }
}
